...
public class MockOtherBundles {
	...
	public MockOtherBundles(ArrayList<String> bundles) throws ClassNotFoundException {
		...
		map.put("wrapperconfig", "/Users/pragyagupta/Projects/SMG_GIT/smg2/config.lib/src/main/java/org/fortiss/smg/config/lib/wrapperconfig.xml");
		...
		if (bundles.contains("Analyzer")) {
			OSGIRunner runnerUser = new OSGIRunner(context,
					new org.fortiss.smg.analyzer.impl.AnalyzerActivator());
			Thread t6 = new Thread(runnerUser);
			t6.run();
			System.out.println("Started: Analyzer");
		}
		
		/*if (bundles.contains("MockActuatorClient")) {
			OSGIRunner runnerUser = new OSGIRunner(context,
					new org.fortiss.smg.actuatorclient.dummy.impl.ActuatorClientActivator());
			Thread t7 = new Thread(runnerUser);
			t7.run();
			System.out.println("Started: MockAcutatorClient");
		}*/
