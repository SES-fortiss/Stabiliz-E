...
public class PersistencyLog implements IActuatorListener , InformationBrokerInterfaceForICTProject {
	...
	@Override
	public void onDoubleEventReceived(DoubleEvent ev, String containerId,
			String client) {
...
*/
			try{
					PreparedStatement query = dbUtil
							.getCon()
							.prepareStatement(
									"INSERT INTO DoubleEvents(containerid, value, maxAbsError, timestamp) VALUES(?,?,?,?)");
					query.setInt(1, Integer.parseInt(containerId));
					query.setDouble(2, ev.getValue());
					if (Double.isNaN(ev.getMaxAbsError())) {
						query.setNull(3, Types.DOUBLE);
					} else {
						query.setDouble(3, Double.isNaN(ev.getMaxAbsError()) ? null
								: ev.getMaxAbsError());
					}
					query.setLong(4, point.getTime());
					query.executeUpdate();
					logger.debug("PersistencyLog: DB Query " + query);
					logger.debug("received new double event. logging to Database "
							+ containerId + "Val: "+ev.getValue() );
					query.close();
			/*	} else {
		...
		*/
			...
			logger.error("Event discarded: doubleEvent({},{})",
					containerId, ev.getValue());
		}
		
	}

	@Override
	public void onComplexDoubleEventReceived(HashMap<String, DoubleEvent> events, String client) throws TimeoutException {
