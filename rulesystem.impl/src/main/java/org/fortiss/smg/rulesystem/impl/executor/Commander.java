...
public class Commander {
	...
	public void setDeviceValue(String containerId, double val ) throws Exception{
		...
		System.out.println ("ContainerID: " + containerId);//"wrapperID: "+ id.getWrapperId()+ "devID: "+ id.getDevid());
		logger.info("ContainerID: " + containerId); //wrapperID: "+ id.getWrapperId()+ "devID: "+ id.getDevid());

		containerManagerClient = clientInfo.init();
		containerManagerClient.sendCommand(new DoubleCommand (command.getValue()), containerId);
		System.out.println("COMMAND:: issued to Device :: value " + command.getValue() + " sent to ContainerID: " + containerId);
...
//		containerManagerClient.sendCommandToContainer(new DoubleCommand(command.getValue()), containerId, SIDeviceType.fromString(type));
	...
	public void recommender(String containerId, double value, String condition) {
		...
		if (answer.startsWith("y")) {
			...
			try {
				setDeviceValue(containerId, value);
			} catch (Exception e) {
				e.printStackTrace();
			}
