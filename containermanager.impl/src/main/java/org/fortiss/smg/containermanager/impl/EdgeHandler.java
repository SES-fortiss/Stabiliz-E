...
public class EdgeHandler {
			...
			if (sqlResult) {
				...
				for (String parent : this.impl.edges.keySet()) {
					...
					if (edgeParent.contains(toCopy)) {
						...
						//now check if it is a real Edge
						for (SingleContainerEdge edge : edgeParent ) {
							if (edge.getType().equals(EdgeType.REAL) && edge.equals(toCopy)) {
								// cleanup & exit
	...
	public List<Entry<Container, EdgeType>> getChildrenWithEdgeTypes(String id) {
		...
		// ArrayList<Pair<Container, EdgeType>>();
		for (SingleContainerEdge edge : current) {
