...
public class BundleFactory {
	...
	public static void activate() throws IOException, TimeoutException {
		...
		RabbitRPCProxy<UserManagerInterface> clientUserManagerInfo = new RabbitRPCProxy<UserManagerInterface>(
				UserManagerInterface.class,
				UserManagerQueueNames.getUserManagerInterfaceQueue(), 5000);

		userManager = clientUserManagerInfo.init();
		...
		RabbitRPCProxy<IActuatorMaster> masterInfo = new RabbitRPCProxy<IActuatorMaster>(
				IActuatorMaster.class,
				ActuatorMasterQueueNames
						.getActuatorMasterInterfaceQueue(), 5000);

		master = masterInfo.init();
		
		RabbitRPCProxy<StabilizeInterface> stabilizeInfo = new RabbitRPCProxy<StabilizeInterface>(
				StabilizeInterface.class,
				StabilizeQueueNames
						.getStabilizeInterfaceQueue(), 5000);

		stabilize = stabilizeInfo.init();
	...
	public static UserManagerInterface getUserManager() {
		return BundleFactory.userManager;
	}
	...
	public static IActuatorMaster getIActuatorMaster() {
		return BundleFactory.master;
	}

	public static StabilizeInterface getStabilize() {
		return BundleFactory.stabilize;
	}
