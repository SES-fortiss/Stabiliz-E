...
public class AnalyzerImpl extends Thread implements AnalyzerInterface {
	...
	@Override
	public Double getSum(long startTime, long stopTime, String containerId)
			throws NoDataFoundException, TimeoutException,
			IllegalArgumentException {
		if (startTime == 0 || stopTime == 0 || containerId == null) {
		...
		DataSet set = new DataSet(startTime, stopTime, containerId);
		try {
	...
	public Double getArithmeticMean(long startTime, long stopTime,
			String containerId) throws TimeoutException, NoDataFoundException,
			IllegalArgumentException {
		if (startTime == 0 || stopTime == 0 || containerId == null) {
			logger.warn("getSum: at least one parameter == null");
			throw new IllegalArgumentException(
	...
	public Double getArithmeticMeanByTime(long startTime,
			long stopTime, String containerId) throws TimeoutException,
	...
	@Override
	public Double getMax(long startTime, long stopTime, String containerId)
	...
	@Override
	public Double getMin(long startTime, long stopTime, String containerId)
			throws IllegalArgumentException, TimeoutException,
			NoDataFoundException {
		if (startTime == 0 || stopTime == 0 || containerId == null) {
			logger.warn("getSum: at least one parameter == null");
	...
	public boolean[] getConsumptionRating(DataSet setCurrent,
		...
		if (setCurrent == null || setReference == null
				...
				|| setCurrent.getContainerId() == null
				|| setReference.getStartDate() == 0
				|| setReference.getStopDate() == 0
				|| setReference.getContainerId() == null) {
	...
	public Double calculateConsumption (long startDate , long lastDate ,DeviceId devId){
		...
		if(devId!=null)
			...
			sql = "SELECT value ,timestamp FROM doubleevents WHERE containerid = '"+ devId.getDevId() +"' AND timestamp >= " + startDate + " AND timestamp <= " +lastDate ;
		try {
